...
class EduDiaryAPI:
    ...
    async def get_marks_in_subject(self, students, subject, class_id=CLASS_ID, period=PERIOD_ID):
    ...
    async def get_final_marks_in_subject(self, students, subject, class_id=CLASS_ID, period=PERIOD_ID):
        async with ClientSession(cookies=self.__cookies__) as ses:
            cookies = {
                'class_id': str(class_id),
                'period': str(period),
            ...
            async with ses.get('http://93.181.225.54/educ_proc/ep_marks/', cookies=cookies, headers=headers) as r:
                ...
                if (len(marks) > len(students)):
                    for i in range(len(students)):
                        marks_row = marks[i].text.strip().split('\n')
                        for m in marks_row:
                            if m == '1' or m == '2' or m == '3' or m == '4' or m == '5' or m == '+':
                                mark = Mark(students[i], mark=m, subject=subject)
                                result.append(mark)
...
async def get_final_marks():
    ...
    subjects = await db.get_subjects()
    students = await db.get_students()
    bar = Bar('Getting final marks', max=len(subjects))
    marks = []
    for subject in subjects:
        api = EduDiaryAPI()
        if subject.id == '202':
            ...
            first = list(filter(lambda x: x.english_group == first, sts))
            second = list(filter(lambda x: x.english_group == second, sts))
            marks.append(await api.get_final_marks_in_subject(students=first, subject=subject, class_id=first[0].english_group, period='i_0'))
            marks.append(await api.get_final_marks_in_subject(students=second, subject=subject, class_id=second[0].english_group, period='i_0'))
        elif subject.id == '31':
            sts = sorted(students, key=lambda x: x.info_group)
            first = sts[0].info_group
            second = sts[len(sts)-1].info_group
            first = list(filter(lambda x: x.info_group == first, sts))
            second = list(filter(lambda x: x.info_group == second, sts))
            marks.append(await api.get_final_marks_in_subject(students=first, subject=subject, class_id=first[0].info_group, period='i_0'))
            marks.append(await api.get_final_marks_in_subject(students=second, subject=subject, class_id=second[0].info_group, period='i_0'))
        elif subject.id == '32':
            sts = sorted(students, key=lambda x: x.gender)
            first = sts[0].gender
            second = sts[len(sts)-1].gender
            first = list(filter(lambda x: x.gender == first, sts))
            second = list(filter(lambda x: x.gender == second, sts))
            marks.append(await api.get_final_marks_in_subject(students=first, subject=subject, class_id=first[0].gender, period='i_0'))
            marks.append(await api.get_final_marks_in_subject(students=second, subject=subject, class_id=second[0].gender, period='i_0'))
        else:
            marks.append(await api.get_final_marks_in_subject(students=students, subject=subject, period='i_0'))
        bar.next()
    bar.finish()
    bar = Bar('Appending new marks', max=len(marks))
    for student in students:
        finals[student.telegram_id] = []
    for mark_row in marks:
        for mark in mark_row:
            finals[mark.student.telegram_id].append(mark)
        bar.next()
    bar.finish()
    return marks